...
async function githubRaw(apiPath) {
  const headers = {
    ...
    'User-Agent': 'neko-runner-dashboard-realtime/4.0',
  };
...
async function runnerState(name) {
  ...
  const value = runner
    ? { found: true, name: runner.name, status: runner.status || 'unknown', busy: typeof runner.busy === 'boolean' ? runner.busy : null }
    : { found: false, name: key, status: 'missing', busy: null };
...
function latestArchivedNodeLog(runnerNames) {
  ...
  try {
    ...
    const stmt = db.prepare(`
      SELECT n.id AS node_id, n.name AS node_name, n.runner_name,
             l.created_at, l.file_name, l.content
      FROM nodes n
      JOIN node_logs l ON l.node_id = n.id
      WHERE n.runner_name = ?
      ORDER BY l.id DESC
      LIMIT 1
    `);
    ...
    console.error(`[logs] SQLite fallback failed: ${err.message}`);
  } finally {
    try { db?.close(); } catch {}
  }
  return null;
...
async function unzipLogs(buffer) {
  ...
  try {
    return await new Promise((resolve, reject) => {
      execFile('unzip', ['-p', tmp], { maxBuffer: 24 * 1024 * 1024 }, (err, stdout, stderr) => {
        if (err) return reject(new Error(stderr || err.message));
        resolve(String(stdout || '').replace(/\u001b\[[0-?]*[ -\/]*[@-~]/g, '').slice(-8 * 1024 * 1024));
      });
    });
  } finally {
...
async function handleRunLogs(req, res, url) {
  ...
  try {
    ...
    console.error(`[logs] run metadata lookup failed: ${err.message}`);
  }
  ...
  try {
    ...
    if (archive.ok) {
      const content = await unzipLogs(Buffer.from(await archive.arrayBuffer()));
      return writeText(res, 200, content || 'GitHub returned an empty workflow log archive.');
    }

    if (archive.status !== 404) {
      const message = await archive.text().catch(() => '');
      return writeText(res, 200,
        `GitHub workflow logs are temporarily unavailable (HTTP ${archive.status}).\n` +
        `Run status: ${run?.status || 'unknown'} / ${run?.conclusion || 'no conclusion'}\n\n` +
        `${message.slice(0, 500)}`
      );
    }
  } catch (err) {
    console.error(`[logs] GitHub archive fetch failed: ${err.message}`);
  }
  ...
  if (fallback) {
    return writeText(res, 200,
      `[GitHub archived logs unavailable]\n` +
...
function handleEvents(req, res) {
  ...
  clients.add(res);
  sendEvent(res, 'ready', { at: new Date().toISOString(), github_refresh_seconds: LIVE_GITHUB_SECONDS });
...
http.createServer = function patchedCreateServer(listener) {
  return originalCreateServer(async (req, res) => {
    ...
    if (pathname === '/internal/nodes/runner-state' && req.method === 'GET') {
      if (!nodeAuthorized(req)) return writeJson(res, 401, { error: 'Invalid node token' });
      try {
        return writeJson(res, 200, await runnerState(url.searchParams.get('name')));
      } catch (err) {
        return writeJson(res, 502, { error: err.message });
      }
    }

    if (pathname === '/api/run-logs' && req.method === 'GET') {
      return handleRunLogs(req, res, url);
    }
    ...
    if (liveEvent) {
      res.once('finish', () => {
        if (res.statusCode >= 200 && res.statusCode < 300) broadcast(liveEvent, { at: new Date().toISOString(), source: pathname });
      });
    }
